...
@interface RegistrationViewController ()<UIGestureRecognizerDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIActionSheetDelegate>
...
- (void)viewDidLoad
{
    ...
    CGRect groupRect = CGRectMake(50, 400, 210, 30);
    ...
    self.groupField.textAlignment = NSTextAlignmentCenter;
...
-(void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if(buttonIndex == 0)
    {
        if(self.chosenImage)
        {
            if(self.firstNameField.text.length != 0 || self.lastNameField.text.length != 0 || self.userField.text.length != 0 || self.passField.text.length != 0 || self.confirmPassField.text.length != 0 || self.emailField.text.length != 0 || self.groupField.text.length != 0)
            {
                [self didAddPlayer];
            }
        }
...
-(void)didAddPlayer
{
    ...
    newUser[@"Group"] = self.groupField.text;
    newUser[@"UserType"] = @"Scout";
